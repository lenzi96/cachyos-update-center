...
from .privilege import authenticate_sudo, clear_auth_cache, elevate_command, get_authenticated_env, is_root
...
class UpdatePipelineWorker(QThread):
    ...
    def run(self):
        ...
        # ----------------------------------------------------------------------
        # Einmalige Vorab-Authentifizierung für die gesamte Update-Pipeline
        # ----------------------------------------------------------------------
        if not is_root():
            ...
            if not auth_ok:
                self.terminal_line.emit("\n❌ Authentifizierung abgebrochen oder fehlgeschlagen. Vorgang wird gestoppt.\n")
        ...
        try:
            ...
            # Securely clear session credentials from memory tmpfs
            clear_auth_cache()
